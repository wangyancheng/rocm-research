...
直到应用程序通知包处理器有新工作可用时，==内核分发包（指AQL 包）==才会启动。通知分为两部分：
...
> 理解：
>
> 1. 释放内存顺序 (Release Memory Order)的作用：执行 `atomic_store_n` 时，硬件确保在此操作之前的**所有内存写入**（那 60 字节内容）必须全部从缓存刷入主存，并对所有代理（Agents）可见。
>
> 2. 门铃机制：`hsa_signal_store_relaxed(queue->doorbell_signal, 0)` 时，写GPU寄存器通知队列0有新工作了，GPU开始从内存中抓取AQL包
>
> 3. 结束同步：当计算单元（CU）完成所有任务，将结果数据从GPU的L2缓存刷入全局显存，将AQL包指定的`completion_signal` 修改为 0。
>
>    

```mermaid
sequenceDiagram
    participant Mem as Queue Ring Buffer (Memory)
    participant Reg as Doorbell Register (MMIO)
    participant Host as CPU (User App)
    participant CP as GPU Command Processor

    Note over Host, Mem: 1. 组装 Packet 其余 60 字节
    Host->>Mem: 写入 Args, Grid Size, Signal 地址等

    rect rgb(230, 245, 255)
    Note over Host, Mem: 2. 原子写入 Header (前 32 位)
    Host->>Mem: atomic_store_release(packet_header)
    Note right of Host: 确保之前 60 字节对 GPU 可见
    end

    rect rgb(255, 245, 230)
    Note over Host, Reg: 3. 通知包处理器 (Doorbell)
    Host->>Reg: hsa_signal_store_relaxed(doorbell, index)
    Note right of Host: 写 MMIO，唤醒 GPU 调度器
    end

    CP->>Reg: 检测到 Doorbell 变化
    CP->>Mem: 根据 index 读取 AQL Packet
    CP->>CP: 解析 Header 类型，开始分发计算
```

| **同步动作**        | **对应物理机制** | **核心目的**                                  |
| ------------------- | ---------------- | --------------------------------------------- |
| **原子写 Header**   | Release Barrier  | 保证包内容的完整性先于包类型的激活。          |
| **写 Doorbell**     | MMIO Trigger     | 越过驱动直接与硬件调度器通信，降低延迟。      |
| **信号等待 (Wait)** | Acquire Barrier  | 保证信号状态的变化先于 CPU 对计算结果的读取。 |
...
==代理之间可以通过使用一致性共享（全局）内存或使用信号进行通信==。代理可以对信号执行类似于对共享内存位置执行的操作。例如，代理可以==原子地==在信号上存储一个整数值，==原子地==加载其当前值等。然而，信号只能使用 HSA 运行时 API 或 HSAIL 指令进行操作。信号相比共享内存的优势在于，信号操作通常在功耗或速度方面表现更好。例如，等待共享内存位置满足条件的自旋循环涉及原子内存操作，可以用 HSA 信号等待操作符（如 `hsa_signal_wait_scacquire`）代替，该操作符由运行时使用高效的硬件特性实现。
...
信号（`hsa_signal_t`）是 HSA 系统中 agent 之间通信的核心原语。本质上是一个包含 64 位有符号整数的原子共享内存对象，任何 agent（CPU 或 GPU）都可以读写。

先看信号的内存结构和两种实现：

![image-20260413132047772](/home/wangyancheng/snap/typora/112/.config/Typora/typora-user-images/image-20260413132047772.png)

信号的核心是一个 64 位原子整数，写入方（GPU 完成后写 0）和读取方（CPU 等待值 < 1）通过这个共享内存位置通信。两种实现的内存布局相同，差异在于等待策略。

接下来看信号在 dispatch 完整生命周期中的角色：

![image-20260413132257106](/home/wangyancheng/snap/typora/112/.config/Typora/typora-user-images/image-20260413132257106.png)
...
1️⃣ 原子操作
   → __atomic_* / GPU atomic 指令

2️⃣ 跨设备可见性
   → PCIe + cache snooping + shared memory

3️⃣ 内存序保证
   → x86 fence（sfence / lfence / mfence）
   → release / acquire 语义
...
**Cache Snooping** 是一种分布式管理机制。它不像有一个中央管理员在分发数据，而是让每个处理器（CPU 或 GPU）的缓存控制器都充当“监听者”。

- 所有缓存控制器都连接在一条**共享总线（Bus）**上。
- 每个控制器都在不断地“窥探（Snooping）”总线上流过的每一个内存请求。

| **状态**          | **含义** | **行为**                                       |
| ----------------- | -------- | ---------------------------------------------- |
| **M** (Modified)  | 已修改   | 数据已改，且只在我这，还没写回主存。           |
| **E** (Exclusive) | 独占     | 数据和主存一致，且只有我有。                   |
| **S** (Shared)    | 共享     | 数据和主存一致，且其他处理器可能也有。         |
| **I** (Invalid)   | 无效     | **核心状态**。数据已过期，不能使用，必须重读。 |

在研究大规模系统时，你会发现两种实现一致性的流派：

**Snooping（窥探）：** 靠“广播”。所有人听广播。

- *优点：* 简单、延迟低。
- *缺点：* 浪费总线带宽。当核心数（如 MI50 的 64 个 CU）非常多时，广播会造成总线拥堵。

**Directory（目录）：** 靠“登记簿”。有一个中央目录记录谁拿了哪份数据。

- *特点：* 适合超多核心的大型服务器，ROCm 在处理多卡间的 **XGMI (Infinity Fabric)** 互联时，更多采用类似目录的探测机制。

![image-20260413144719662](/home/wangyancheng/snap/typora/112/.config/Typora/typora-user-images/image-20260413144719662.png)

保障来自三层叠加，缺一不可。接下来看内存序这个最容易忽视的层：

![image-20260413145443620](/home/wangyancheng/snap/typora/112/.config/Typora/typora-user-images/image-20260413145443620.png)

最后看 GPU 侧的原子操作是怎么实现的，这是和 CPU 原子操作最不同的地方：

![image-20260413145533354](/home/wangyancheng/snap/typora/112/.config/Typora/typora-user-images/image-20260413145533354.png)

* Fine-grained：主要用于信号，内核参数，频繁读写的小数据结构，有硬件缓存一致性协议，无需软件flush。

* Coarse-grained：主要用于大块数据缓冲区，无硬件缓存一致性协议，需要显式刷新。

## 三层保障的本质

**第一层：硬件原子指令** CPU 侧用 `LOCK XCHG`/`LOCK CMPXCHG`（x86）或 `LDXR/STXR`（ARM），GPU 侧用 `flat_atomic_*` 系列 GCN 指令。共同保证：读-改-写这三步在硬件层面不可分割，中间不会被其他核插入。

**第二层：内存序（最容易踩坑）** 原子性只保证操作本身不被打断，不保证其他内存操作的可见顺序。`release` 的含义是：store 之前的所有写操作，对执行 `acquire` load 的一方都可见。这就是为什么 GPU 写 `signal_value = 0` 必须用 release 语义——它要保证 kernel 写的计算结果在这条写之前已经对 CPU 可见。

你代码里 AQL packet header 用了 `__ATOMIC_SEQ_CST`（值为 3），比 release 更强，是全局全序，代价是完整的内存屏障指令。

**第三层：共享内存页** ROCr 用 `mmap` 把 signal 的 `signal_value_` 字段映射到一段 CPU 和 GPU 都能访问的物理内存页（fine-grained memory）。GPU 的 `flat_atomic` 指令 bypass L1 直达 L2，再写入系统内存，CPU 通过 `acquire load` 直接读到最新值，不需要显式的 cache flush。

如果 signal 放在 coarse-grained memory（普通 VRAM），GPU 写完后 CPU 不会自动看到，必须手动 flush L2，这就是为什么 ROCr 内部对 signal 的内存分配有专门的 `fine-grained` 路径。

最后是信号与 HSA 其他同步机制的横向对比：

![image-20260413132518793](/home/wangyancheng/snap/typora/112/.config/Typora/typora-user-images/image-20260413132518793.png)

## 四种机制的核心区别

**Signal** 是通用双向原语，CPU 和 GPU 都能读写，支持 `store`、`wait`、`add`、`cas` 等原子操作，并带内存序语义（`relaxed` / `acquire` / `release`）。你代码里的 `completion_signal` 就是这个。

**Fence** 不是独立对象，是 AQL packet header 里的两个字段，控制 kernel 执行前后的内存可见性范围。`SYSTEM` 范围最强但代价最高，`AGENT` 只保证同一 GPU 内可见，`NONE` 不保证。你的代码用了 `SYSTEM`，是最保守安全的选择。

**Barrier packet** 是一种特殊的 AQL packet，放在 queue 里，让 GPU 自己等待最多 5 个前置 signal 完成后再继续消费后续 packet。核心优势是 CPU 完全不参与同步，适合多 kernel 依赖链。

**Doorbell signal** 看起来像 signal，但实质是 MMIO 映射，写入操作直接触发硬件，不经过内核，延迟极低。写的值是 write index，不是任意整数。单向，只用于 CPU 通知 GPU 有新包。
...
sequenceDiagram
    ...
    participant RB as Ring Buffer (AQL Queue)
    participant DB as Doorbell (GPU MMIO)
    participant CP as GPU Command Processor (Hardware)
    participant CU as 计算单元 (Compute Units)

    Note over App, RB: 1. 预留槽位 (Slot Acquisition)
    App->>App: 获取当前 Write Index (atomic_add)
    
    Note over App, RB: 2. 写入数据 (Packet Submission)
    App->>RB: 填充 64字节 AQL Packet
    Note right of App: 包含 Header, Kernel Addr, Args...
    
    Note over App, DB: 3. 敲门通知 (Doorbell Ring)
    App->>DB: 更新 Doorbell (写入新 Index)
    
    Note over CP, RB: 4. 抓取指令 (Fetch)
    CP->>RB: 根据 Read Index 抓取 Packet
    
    Note over CP, CU: 5. 任务分发 (Dispatch)
    CP->>CP: 解析 Packet 类型
    CP->>CU: 派发 Wavefronts 执行计算
    
    Note over CP, RB: 6. 推进读指针
    CP->>CP: 更新 Read Index (Read == Write 时回空闲)
```
...
| Memory Offset | Field Name               | Size (Bytes) | Description                 |
| ------------- | ------------------------ | ------------ | --------------------------- |
| 0             | header                   | 2            | Packet type & Barriers      |
| 2             | setup                    | 2            | Dimensions (1D/2D/3D)       |
| 4             | workgroup_size_x/y/z     | 6            | Thread count per WG         |
| 10            | (reserved)               | 2            | Padding                     |
| 12            | grid_size_x/y/z          | 12           | Total threads               |
| 24            | ==private_segment_size== | 4            | Per-thread scratch memory   |
| 28            | ==group_segment_size==   | 4            | Per-WG LDS memory           |
| 32            | (reserved)               | 8            | Padding                     |
| 40            | ==kernel_object==        | 8            | Address of ISA instructions |
| 48            | ==kernarg_address==      | 8            | Address of Kernel arguments |
| 56            | completion_signal        | 8            | Signal to update on finish  |
-----------------------------------------------------------------------------------
Total Size: 64 Bytes (Matches Cache Line)
...
> 屏障-AND (Barrier-AND) 包：提交4个并行Kernel，当所有Kernel完成后，才执行后面的工作。
>
> 屏障-OR (Barrier-OR) 包：两个Agent执行搜索任务，任何一个完成，继续执行后面的工作。
...
![image-20260413163014384](/home/wangyancheng/snap/typora/112/.config/Typora/typora-user-images/image-20260413163014384.png)
...
```mermaid
graph TD
    subgraph "Logic Segments (软件逻辑层)"
        S1[Kernarg Segment]
        S2[ReadOnly Segment]
        S3[Global Segment]
        S4[Group Segment - LDS]
        S5[Private Segment]
    end

    subgraph "Physical Memory (物理存储层)"
        HostRAM[(Host RAM - 系统内存)]
        HBM2[(HBM2 - 显存)]
    end

    subgraph "GPU Hardware - MI50 (硬件处理层)"
        CP[Command Processor]
        L2[L2 Cache]
        
        subgraph "Compute Unit (CU)"
            SC[Scalar L1 Cache]
            VC[Vector L1 Cache]
            LDS[LDS RAM]
            SGPR[标量寄存器 SGPR]
    ...
    %% Kernarg 路径
    S1 -- "Initial Write" --> HostRAM
    HostRAM -- "Pre-fetch" --> CP
    CP -- "Dispatch" --> SGPR
    ...
    style SGPR fill:#f96,stroke:#333
...
%% =====================
subgraph SYS_MEM
C[System RAM<br>Fine grained memory<br>Signal lives here]
end
...
D[Compute Unit]
E[GPU L1 Cache]
F[GPU L2 Cache<br>Coherent point]
end
...
**Global segment**：最大的段，所有 agent 都能访问，用于内核参数、输入输出数据。你代码里的 `global_pool` 就属于这里。生命周期由程序控制。

**Group segment**（LDS）：每个 workgroup 私有的片上内存，相当于 CUDA 的 shared memory。延迟极低（~4 cycle），但容量小（MI50 上 64KB/CU）。不能跨 workgroup 访问，kernel 结束即释放。

**Private segment**（Scratch）：每个 wavefront 线程私有，用于寄存器溢出。实际是 VRAM 里的一段区域，访问慢，编译器尽量避免使用。

**Kernarg segment**：专门存放 kernel 参数，只读，dispatch 前由 CPU 填写，GPU 执行时通过 SGPR 加载。你代码里单独分配的 `kernarg_gpu` 就是这个段。

### 二、内存区域（Region/Pool）属性

段是逻辑分类，实际分配走的是 `hsa_amd_memory_pool_t`，每个 pool 有两个关键属性：

**粒度（Granularity）**，分两种：

`fine-grained`：CPU 和 GPU 对这段内存的写操作对彼此立即可见，不需要显式 flush。实现上走的是绕过 GPU L2 cache 直写系统内存的路径。Signal 的 `signal_value_` 字段必须在这里。代价是每次访问都走 PCIe，GPU 访问延迟高。

`coarse-grained`：GPU 写完后 CPU 不保证立即看到，需要显式同步（`hsa_memory_copy` 或 barrier）才能看到最新值。但 GPU 访问走本地 VRAM，延迟低、带宽高。你代码里的 `out_gpu` 和 `kernarg_gpu` 属于这种。

**访问权限**：通过 `hsa_amd_agents_allow_access` 控制哪些 agent 能访问。底层是建立 GPU 页表映射。

### 三、MI50 的地址空间布局

MI50（gfx906）的 GPUVM 地址空间由 thunk 的 `fmm.c` 管理：

```
高地址
┌─────────────────────────┐
│  Scratch 孔径            │  ← Private segment，寄存器溢出
│  (~0xE000_0000_0000)    │
├─────────────────────────┤
│  LDS 孔径               │  ← Group segment，片上，无实际 VA
├─────────────────────────┤
│  GPUVM 用户空间          │  ← Global segment，hsa_amd_memory_pool_allocate
│  (~0x7F00_0000_0000)    │     返回的 VA 在这个范围内
├─────────────────────────┤
│  Kernarg 孔径            │  ← Kernarg segment，特殊对齐要求
│  (~0x0000_0000_0000)    │
低地址
```
...
ROCr 启动时通过 sysfs 读取拓扑，建立 pool 列表：

```mermaid
graph TD
    %% 第一阶段：内核探测
    subgraph KFD_Layer [1. Kernel Layer - 拓扑探测]
        KFD[/sys/class/kfd/kfd/topology/nodes/] --> Node_Parse[解析 Node 属性]
        Node_Parse --> Mem_Banks[获取 mem_banks: VRAM / System]
    end

    %% 第二阶段：运行时初始化
    subgraph HSA_Runtime [2. HSA Runtime - hsa_init]
        Pool_Gen{内存池分类映射}
        
        Mem_Banks --> Pool_Gen
        
        Pool_Gen -- "Local VRAM (Non-Coherent)" --> Coarse[Global Coarse-grained Pool<br/>'PUBLIC, VRAM']
        Pool_Gen -- "Host RAM (Coherent)" --> Fine[Global Fine-grained Pool<br/>'PUBLIC, SYSTEM']
        Pool_Gen -- "Private/Scratch" --> Scratch[Scratch Pool<br/>'PRIVATE']
    end

    %% 第三阶段：应用层
    subgraph App_Layer [3. Application Layer - 内存请求]
        Iterate[hsa_amd_agent_iterate_memory_pools]
        Alloc[hsa_amd_memory_pool_allocate]
        
        Coarse -.-> Iterate
        Fine -.-> Iterate
        Scratch -.-> Iterate
        
        Iterate --> User_Logic{选择逻辑}
        User_Logic -- "计算密集" --> Alloc_Coarse[分配到 VRAM]
        User_Logic -- "信号/小交互" --> Alloc_Fine[分配到 System]
    end

    %% 物理链路标注
    Alloc_Coarse ===>|High BW| HBM2((MI50 HBM2))
    Alloc_Fine ===>|PCIe/XGMI| DDR((Host DDR))

    style Coarse fill:#f96,stroke:#333
    style Fine fill:#6cf,stroke:#333
    style Scratch fill:#9c6,stroke:#333
```

### 五、分配路径

一次 `hsa_amd_memory_pool_allocate` 的完整路径：

```mermaid
graph TD
    %% 用户态层
    subgraph User_Space [用户态 User Space]
        API[hsa_amd_memory_pool_allocate] --> Region[ROCr: MemoryRegion::Allocate]
        Region --> Thunk[Libhsakmt: hsaKmtAllocMemory]
    end

    %% 系统调用分界线
    Thunk -- "ioctl (ALLOC_MEMORY)" --> KFD_IOCTL

    %% 内核态层
    subgraph Kernel_KFD [内核 KFD 驱动]
        KFD_IOCTL[kfd_ioctl_alloc_memory_of_gpu] --> KFD_VM[amdgpu_amdkfd_gpuvm_alloc_memory_of_gpu]
    end

    subgraph Kernel_AMDGPU [内核 AMDGPU 驱动 / TTM]
        KFD_VM --> Type_Check{内存类型判断}
        
        Type_Check -- "VRAM Pool" --> BO_VRAM[amdgpu_bo_create<br/>在物理显存创建 BO]
        Type_Check -- "GTT/Fine-grained" --> BO_GTT[amdgpu_bo_create<br/>在系统内存创建 BO]
        
        BO_VRAM --> VA_Assign[分配 GPUVM 虚拟地址]
        BO_GTT --> VA_Assign
    end

    %% 返回与后续映射
    VA_Assign -- "返回 VA 指针" --> API

    %% 映射阶段
    subgraph Memory_Mapping [内存映射与授权]
        API_Map[hsa_amd_agents_allow_access] -- "ioctl (MAP_MEMORY)" --> KFD_MAP[kfd_ioctl_map_memory_to_gpu]
        KFD_MAP --> VM_Update[amdgpu_vm_bo_update<br/>更新硬件页表 PTE]
    end

    %% 硬件层
    subgraph Hardware [GPU 硬件层]
        VM_Update --> GPU_MMU[GPU MMU / IOMMU]
        GPU_MMU -.-> Access[GPU 核心通过 VA 访问内存]
    end

    style Type_Check fill:#f9f,stroke:#333
    style BO_VRAM fill:#f96,stroke:#333
    style BO_GTT fill:#6cf,stroke:#333
    style VM_Update fill:#9c6,stroke:#333
```
...
> 细粒度全局区域 (System-Wide Fine-Grained)：在主机内存，硬件自动一致性，Cache Snooping，主要用于存放信号。
>
> Kernarg 后备存储区域 (Kernarg Backing Store)：专门用于存放参数，允许在某后自动进行分配

```mermaid
graph LR
    subgraph "Host Memory (DDR)"
        FG_Global[细粒度全局区域<br/>Signal / Data Sharing]
        Kernarg_Store[Kernarg 后备区域<br/>Parameters Storage]
    end

    subgraph "Device Memory (VRAM)"
        Coarse_Global[粗粒度全局区域<br/>Massive Data]
    end

    GPU_Agent((GPU Agent)) -- "Atomic Snoop" --> FG_Global
    GPU_Agent -- "Direct Load" --> Kernarg_Store
    GPU_Agent -- "High Bandwidth" --> Coarse_Global
```
...
> ==hsa_memory_register：把当前内存设置为Fine-Grained（细粒度）。==
...
一切的起点是 `.hsaco` 文件（或内存中的二进制流）。

- **(1) 编译产物：** 开发者通过离线编译器（如 `amdclang++`）将代码编译成包含 GPU ISA 指令的 ELF 格式代码对象。它可以存在于**磁盘文件**中，也可以通过 JIT 编译存在于**内存**中。
- **(2) 读取器初始化：** * 使用 `hsa_code_object_reader_create_from_file` 或 `_from_memory`。
  - **本质：** 这一步并没有加载指令，而是创建了一个**句柄**，让运行时知道去哪里解析这个 ELF 文件的符号表和段信息。

------

### 第二阶段：构建可执行体容器 (3) & (4)

有了读取器后，我们需要一个“容器”来存放这些即将运行的指令。

- **(3) 创建 Executable：** `hsa_executable_create_alt` 创建一个逻辑上的容器。
  - 它定义了执行的环境（如 Profile、默认内存池等）。
- **(4) 内存驻留与重定位：** 调用 `hsa_executable_load_agent_code_object`。
  - **核心动作：** 这是**最重**的操作。运行时会真正分配 GPU VRAM，将代码段（.text）从文件拷贝到显存。
...
现在指令已经躺在显存里了，我们需要找到它的“大门”。

- **(5) 符号检索：** `hsa_executable_get_symbol_by_name`。
  - 你告诉它函数名（字符串，如 `"my_kernel"`），它在可执行体的符号表中检索出对应的 `hsa_executable_symbol_t` 句柄。
- **(6) 提取内核对象：** `hsa_executable_symbol_get_info`。
  - **最终产物：** 提取 `HSA_EXECUTABLE_SYMBOL_INFO_KERNEL_OBJECT`。
  - **结果：** 这是一个 64 位的 **GPU 虚拟地址**。
...
| **步骤**    | **对应图中的节点**     | **物理意义**                              |
| ----------- | ---------------------- | ----------------------------------------- |
| **Source**  | Code Object (File/Mem) | 磁盘或内存上的静态二进制数据 (ELF)        |
| **Carrier** | Code Object Reader     | 文件解析句柄，用于读取符号                |
| **Storage** | hsa_executable_t       | **显存中的指令流**，已经完成了重定位      |
| **Target**  | **Kernel Object**      | **第一条指令的绝对地址**，准备填入 AQL 包 |

```c++
// 1. 初始化与探测
hsa_init();
hsa_agent_iterate_agents(find_gpu_agent, &gpu_agent);

// 2. 内存准备 (Global Segment)
hsa_amd_memory_pool_allocate(cpu_pool, size, ..., &data_in_cpu);
hsa_amd_memory_pool_allocate(gpu_pool, size, ..., &data_out_gpu);
hsa_amd_agents_allow_access(1, &gpu_agent, ..., data_in_cpu); // 允许 GPU 访存

// 3. 加载代码对象 (对应你图中的流程)
hsa_code_object_reader_create_from_file(file_handle, &reader);
hsa_executable_create_alt(HSA_PROFILE_FULL, ..., &executable);
hsa_executable_load_agent_code_object(executable, gpu_agent, reader, ...);
hsa_executable_get_symbol_by_name(executable, "vector_copy_kernel", ..., &symbol);
hsa_executable_symbol_get_info(symbol, HSA_EXECUTABLE_SYMBOL_INFO_KERNEL_OBJECT, &kernel_obj);

// 4. 创建指令队列与提交
hsa_queue_create(gpu_agent, 32, ..., &queue);
...
// 填充 AQL 包
packet->kernel_object = kernel_obj; // 关键：填入图中获取的入口地址
packet->kernarg_address = args_buffer;
packet->grid_size_x = 1024;
packet->workgroup_size_x = 256;
packet->header = ... | HSA_PACKET_TYPE_KERNEL_DISPATCH; // 激活包
